      ...
      VirtualSpace.create({
        ...
        .then(({ virtual_space }) => {
          ...
          return VirtualSpace.join(virtual_space._id.toString())
            .then(({ message, virtual_space }) => {
              VirtualSpace.getSocketClients(
                NameSpace.in(VirtualSpace._id)
              ).then((viewers) => {
                const { users } = viewers;
                NameSpace.to(VirtualSpace._id).emit("viewers", viewers);
              ...
              addTagToVirtualSpace({
                text: "Desktop",
                description: "For work purposes",
                position: {
                  x: 4.152099023990676,
                  y: 2.0982018461820537,
                  z: 2.1693910252107926,
                },
              });
      ...
      if (User.socket_id === VirtualSpace.creator_id || VirtualSpace.master) {
        VirtualSpace.updateAttributes(attributes)
          .then((virtual_space) => {
            ...
            NameSpace.to(VirtualSpace._id).emit("attributes", {
              virtual_space,
            });
          })
          .catch((err) => errorHandler(err, socket));
